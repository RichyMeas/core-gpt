...
def initalize_logging(master_process, cfg):
    ...
    tb_writer = None # Define tb_writer outside the if block
    if master_process:
        ...
        tb_writer = SummaryWriter(log_dir=experiment_dir_path / "tensorboard")
        ...
        print0(f"Tensorboard: {experiment_dir_path / 'tensorboard'}", master_process, logfile, console=True)
        ...
        tokenizer_path = Path(f"tokenizers/{cfg.tokenizer}")
    ...
    return logfile, experiment_dir_path, metrics_csv_path, tb_writer